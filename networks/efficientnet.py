...
class EffNet(Module):
    ...
    def __init__(self, width_mult=1.0, params=None):
        super(EffNet, self).__init__()
        self.params = EffNet.get_params() if not params else params
        vs = ["sw", "ds", "ws", "exp_rs", "se_r", "ss", "ks", "hw", "nc"]
        sw, ds, ws, exp_rs, se_r, ss, ks, hw, nc = [self.params[v] for v in vs]
        se_r /= width_mult

        ws = [make_divisible(w * width_mult, cfg.SCALING.ROUND) for w in ws]
        sw = make_divisible(sw * width_mult, cfg.SCALING.ROUND)
        hw = make_divisible(hw * width_mult, cfg.SCALING.ROUND)
        ws, exp_rs, _ = bk.adjust_block_compatibility(ws, exp_rs, [1] * len(ds))
        self.params['ds'], self.params['ws'], self.params['sw'], self.params['hw'], self.params['exp_rs'] = ds, ws, sw, hw, exp_rs
...
if __name__ == '__main__':
    config.load_cfg('../configs/effnets/effnet_b0.yaml')
    cfg.freeze()

    model = EffNet(width_mult=1.0)
    inputx = torch.randn(1, 3, 224, 224)
    model, inputx = model.cuda(), inputx.cuda()
    model.eval()
    with torch.no_grad():
        output = model(inputx)
    print(model)
