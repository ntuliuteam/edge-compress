...
def conv2d_1x1(w_in, w_out, stride=1, bias=False):
    """Helper for building a conv2d layer with 1x1 kernel."""
    return conv2d(w_in, w_out, 1, stride=stride, bias=bias)


def conv2d_3x3(w_in, w_out, stride=1, groups=1, bias=False):
    """Helper for building a conv2d layer with 3x3 kernel."""
    return conv2d(w_in, w_out, 3, stride=stride, groups=groups, bias=bias)
...
def linear(w_in, w_out, *, bias=True):
...
def conv2d_1x1_cx(cx, w_in, w_out, stride=1):
    """Accumulates complexity of conv2d with 1x1 kernel into cx = (h, w, flops, params, acts)."""
    return conv2d_cx(cx, w_in, w_out, 1, stride=stride)


def conv2d_3x3_cx(cx, w_in, w_out, stride=1, groups=1):
    """Accumulates complexity of conv2d with 3x3 kernel into cx = (h, w, flops, params, acts)."""
    return conv2d_cx(cx, w_in, w_out, 3, stride=stride, groups=groups)
...
class ConvNormActivation(torch.nn.Sequential):
    """Taken directly from torchvision official github repository:
    https://github.com/pytorch/vision/blob/v0.13.0/torchvision/ops/misc.py"""
    def __init__(
        self,
        in_channels: int,
        out_channels: int,
