...
class BlockParams:
    ...
    def from_init_params(
        ...
        **kwargs: Any,
        ...
        stage_widths = [make_divisible(w * width_mult, cfg.SCALING.ROUND) for w, t in zip(block_widths, splits[:-1]) if t]
        stage_depths = torch.diff(torch.tensor([d for d, t in enumerate(splits) if t])).int().tolist()

        strides = [STRIDE] * num_stages
        bottleneck_multipliers = [bottleneck_multiplier] * num_stages
        group_widths = [make_divisible(group_width * width_mult, 2)] * num_stages
...
def regnet_x_800mf(pretrained: bool = False, progress: bool = True, **kwargs: Any) -> RegNet:
    ...
    Args:
        ...
        progress (bool): If True, displays a progress bar of the download to stderr
    """
    params = BlockParams.from_init_params(depth=16, w_0=56, w_a=35.73, w_m=2.28, group_width=16,
                                          width_mult=kwargs.pop("width_mult"), **kwargs)
...
if __name__ == "__main__":
    ...
    model = regnet_x_800mf(width_mult=1.0)
